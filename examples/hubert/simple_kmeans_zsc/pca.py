...
def get_parser():
    ...
    # fmt: off
    parser.add_argument('data', help='all numpy file containing features separated by commas')
...
def load_feature_shard(feat_path):
    leng_path =feat_path.replace(".npy",".len")
...
def main():
    ...
    for filename in os.listdir(args.data):
        if(filename.endswith(".npy")):
            ...
            aux, _ = load_feature_shard(os.path.join(args.data, os.path.join(filename)))
            feat_list += aux
            del aux
