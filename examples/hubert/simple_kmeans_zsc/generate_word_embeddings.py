...
def load_feature_shard(feat_dir, split, nshard, rank):
    ...
    with open(leng_path, "r") as f:
        ids = [line.rstrip().split()[0].split("/")[-1].replace(".wav","").replace(".flac","") for line in f]
        ...
        for idx, key in tqdm.tqdm(enumerate(shard_ids), total=len_shard):
            ...
            for word_index in range(len(word_segments)):
                ...
                if(word == "" or word == "SIL"):
                    ...
                    elif(pooling_type == "one"):
                        middle=int((max_sample-min_sample)/2)
                        awe = mat[middle,:]
                    

                    real_start, real_end = real_start_end[word_index]
                    if(setname == "mandarin"):
                        key_id=key.strip().split("_")[0]
                        npz_id = word + "_" + key_id +"_%06d-%06d" % (int(round(real_start*100)),int(round(real_end*100)) + 1)
                    elif(setname == "french"):
                        key_id=key.strip().split("_")[0]+"_"+key.strip().split("_")[1]
                        npz_id = word + "_" + key_id +"_%06d-%06d" % (int(round(real_start*100)),int(round(real_end*100)) + 1)
                    else:
