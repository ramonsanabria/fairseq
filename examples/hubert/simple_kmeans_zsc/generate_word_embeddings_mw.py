...
def get_alignmens_segment(alignments_file, idx_segment):
    ...
    return min_time, words_in_segments
...
if("a" in margin):
    margin=int(margin.replace("a",""))
elif("s" in margin):
    margin=-int(margin.replace("s",""))
...
with NpyAppendArray(feature_path) as npaa, open(word_path,"w") as wordfile:
    ...
    for r in range(nshard):
        ...
        for idx, key in tqdm.tqdm(enumerate(shard_ids), total=len_shard):
            ...
            min_time, word_segments = get_alignmens_segment(alignments_file, idx_segment)
            ...
            for word_index in range(len(word_segments)):
                ...
                if(word == "" or word == "SIL"):
                    ...
                    min_sample = int(((start-min_time)*1000)/20)-int((margin*100)/20)
                    max_sample = int(((end-min_time)*1000)/20)+int((margin*100)/20)
                    #print("AFTER: "+str(mat.shape))
                    #print("min_sample: "+str(min_sample))
                    #print("max_sample: "+str(max_sample))
                    #print("SUBSTRACT: "+str())
                    ...
                    if(max_sample >= (mat.shape[0]-1)):
                        max_sample=mat.shape[0]-1

                    if((max_sample - min_sample) <= 0):
                        continue

                    #print("FINAL: "+str(mat.shape))
                    #print("min_sample: "+str(min_sample))
                    #print("max_sample: "+str(max_sample))
                    #print("RANGE: "+str(mat.shape))
                    #print("")
                    #print("")
                    #print("")
